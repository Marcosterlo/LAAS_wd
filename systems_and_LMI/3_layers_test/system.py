...
class System:
  ...
  def __init__(self):
    ...
    W1_name = os.path.abspath(__file__ + "/../3_layers/W1.csv")
    W1 = np.loadtxt(W1_name, delimiter=',')
    W2_name = os.path.abspath(__file__ + "/../3_layers/W2.csv")
    W2 = np.loadtxt(W2_name, delimiter=',')
    W3_name = os.path.abspath(__file__ + "/../3_layers/W3.csv")
    W3 = np.loadtxt(W3_name, delimiter=',')
    W4_name = os.path.abspath(__file__ + "/../3_layers/W4.csv")
    ...
    b1_name = os.path.abspath(__file__ + "/../3_layers/b1.csv")
    b1 = np.loadtxt(b1_name, delimiter=',')
    b2_name = os.path.abspath(__file__ + "/../3_layers/b2.csv")
    b2 = np.loadtxt(b2_name, delimiter=',')
    b3_name = os.path.abspath(__file__ + "/../3_layers/b3.csv")
    b3 = np.loadtxt(b3_name, delimiter=',')
    b4_name = os.path.abspath(__file__ + "/../3_layers/b4.csv")
  ...
  def forward(self, x):
    ...
    nu = self.tanh(self.layers[0](torch.tensor(x)))
    nu = self.tanh(self.layers[1](nu))
    nu = self.tanh(self.layers[2](nu))
    nu = self.layers[3](nu).detach().numpy()

    return nu*self.max_torque
  ...
  def dynamic_loop(self, x0, nstep):
    ...
    for i in range(nstep):
      ...
      # Forward dynamics
      x = (self.A + self.B @ self.K) @ self.state + self.B @ u
