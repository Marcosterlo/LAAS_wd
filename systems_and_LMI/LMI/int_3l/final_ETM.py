...
class LMI_final():
  def __init__(self, W, b):
    ...
    self.T_sat = cp.Variable((self.nu, self.nu))
    ...
    self.Z_sat = cp.Variable((self.nu, self.nx))
    ...
    self.Rnu = cp.bmat([
      [np.eye(self.nx), np.zeros((self.nx, self.nphi)), np.zeros((self.nx, self.nu)), np.zeros((self.nx, self.nq))],
      [np.zeros((self.nphi, self.nx)), np.eye(self.nphi), np.zeros((self.nphi, self.nu)), np.zeros((self.nphi, self.nq))],
      [self.R @ self.Nvx, np.eye(self.R.shape[0]) - self.R, np.zeros((self.nphi, self.nu)), np.zeros((self.nphi, self.nq))],
    ])

    self.Rs = cp.bmat([
      [np.eye(self.nx), np.zeros((self.nx, self.nphi)), np.zeros((self.nx, self.nu)), np.zeros((self.nx, self.nq))],
      [np.zeros((self.nu, self.nx)), np.zeros((self.nu, self.nphi)), np.eye(self.nu), np.zeros((self.nu, self.nq))],
      [self.Rw, -self.Nuw @ self.R, np.zeros((self.nu, self.nu)), np.zeros((self.nu, self.nq))],
    ])
    ...
    self.Rphi = cp.bmat([
      [self.K, np.zeros((1, self.nphi)), np.zeros((1, self.nu)), np.zeros((1, self.nq))],
      [np.zeros((self.nq, self.nx)), np.zeros((1, self.nphi)), np.zeros((self.nq, self.nu)), np.eye(self.nq)]
    ])
    ...
    self.M1 = cp.bmat([
      [self.Abar.T @ self.P @ self.Abar - self.P, self.Abar.T @ self.P @ self.Bbar, self.Abar.T @ self.P @ self.B, self.Abar.T @ self.P @ self.C],
      [self.Bbar.T @ self.P @ self.Abar, self.Bbar.T @ self.P @ self.Bbar, self.Bbar.T @ self.P @ self.B, self.Bbar.T @ self.P @ self.C],
      [self.B.T @ self.P @ self.Abar, self.B.T @ self.P @ self.Bbar, self.B.T @ self.P @ self.B, self.B.T @ self.P @ self.C],
      [self.C.T @ self.P @ self.Abar, self.C.T @ self.P @ self.Bbar, self.C.T @ self.P @ self.B, self.C.T @ self.P @ self.C]
    ])
    

    if self.xETM:
      self.M2 = self.Rnu.T @ (self.R1.T @ (self.gamma1_scal * (self.bigX1 + self.bigX1.T)) @ self.R1 + self.R2.T @ (self.gamma2_scal * (self.bigX2 + self.bigX2.T)) @ self.R2 + self.R3.T @ (self.gamma3_scal * (self.bigX3 + self.bigX3.T)) @ self.R3) @ self.Rnu
    else:
      self.M2 = self.Rnu.T @ (self.R1.T @ (self.gamma1_scal * (self.Omega1 + self.Omega1.T)) @ self.R1 + self.R2.T @ (self.gamma2_scal * (self.Omega2 + self.Omega2.T)) @ self.R2 + self.R3.T @ (self.gamma3_scal * (self.Omega3 + self.Omega3.T)) @ self.R3) @ self.Rnu
